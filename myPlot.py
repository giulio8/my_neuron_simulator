...
def myPlotFigure (common_axis:plt.Axes, xaxis, yaxis, title, unitLabel="", labels=None, nplots=1):
    if (nplots > 1):
        for i in range(nplots):
            if (labels != None):
                y = yaxis[i] if type(yaxis) == list or len(yaxis.shape) == 1 else yaxis[i, :]
...
def drawFigure(simulation_time, time_step, variables: neuron.Variables, time_window=None, n_drawings=1, n_plots=1, axes=None, labels=None, dpixel=400):
    ...
    effective_simulation_time = end - start
    ...
    for ax, (key, variable) in zip(axes.flat, variables_to_plot):
        myPlotFigure(ax, t_axis, variable.value, variable.title, variable.unit, labels=labels, nplots = n_plots)

    plt.show()

    if (n_plots == 1):
        if (variables_to_plot.S.array == True):
            print("Average number of APs: %.1f (%.1f Hz)"%(np.sum(variables_to_plot.S.value), np.sum(variables_to_plot.S.value)/effective_simulation_time))
        if (variables_to_plot.release_vector.array == True):
            print("Average number of vesicle releases: %.1f (%.1f Hz)"%(np.sum(variables_to_plot.release_vector.value), np.sum(variables_to_plot.release_vector.value)/effective_simulation_time))
        if (variables_to_plot.S.array == True):
            print("Average mutual information: %.4f bit/s"% (np.sum(variables_to_plot.mutual_information.value)/(effective_simulation_time/time_step))) # Bitrate already has s^(-1)
    else:
        if (variables_to_plot.S.array == True):
            for i, value in enumerate(variables_to_plot.S.value):
                print("Average number of APs: %.1f (%.1f Hz)"%(np.sum(value), np.sum(value)/effective_simulation_time))
                print(labels[i])
        if (variables_to_plot.release_vector.array == True):
            for i, value in enumerate(variables_to_plot.release_vector.value):
                print("Average number of vesicle releases: %.1f (%.1f Hz)"%(np.sum(value), np.sum(value)/effective_simulation_time))
                print(labels[i])
        if (variables_to_plot.S.array == True):
            for i, value in enumerate(variables_to_plot.mutual_information.value):
                print("Average mutual information: %.4f bit/s"% (np.sum(value)/(effective_simulation_time/time_step))) # Bitrate already has s^(-1)
                print(labels[i])